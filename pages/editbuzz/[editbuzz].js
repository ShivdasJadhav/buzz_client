...
import { getBuzz, updateBuzz } from "../api/operateBuzz";
import Link from "next/link";
import { useRouter } from "next/router";
function editbuzz() {
  let router = useRouter();
  let id = router.query.editbuzz;
  const getData = async () => {
    if (id) {
      setBuzz(await getBuzz(id).then((res) => res));
    }
  };
  useEffect(() => {
    getData();
  ...
  const saveBuzz = async () => {
    let res = await updateBuzz(buzz, id).then((res) => res);
    if (res === "ok") {
      router.push("/");
    ...
    <div className="w-11/12 mx-auto">
      ...
      <div className="w-full md:w-6/12 mx-auto py-6 flex flex-col border border-2 border-gray-300 rounded-xl">
        ...
        <div className="w-fit mx-auto">
          <Link
            href={"/"}
            className="mr-2 w-32 text-center inline-block border border-gray-300 border-2 rounded-md py-1 px-3 bg-gray-800"
          >
            Cancel
          </Link>
          <button
